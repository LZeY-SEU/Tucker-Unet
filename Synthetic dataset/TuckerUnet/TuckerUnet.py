...
from TuckerUnet.attend import Attend
...
class FiLMedConv(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, padding=0, time_emb_dim=None):
        super().__init__()
    ...
    def forward(self, x, time_emb=None):
        x = self.conv(x)  # [B, C_in, H, W] → [B, out_channels, H, W]
        if exists(self.film_mlp) and exists(time_emb):
            film_params = self.film_mlp(time_emb)                     # [B, out_channels*2]
            film_params = rearrange(film_params, 'b c -> b c 1 1')    # [B, out_channels*2, 1, 1]
            gamma, beta = film_params.chunk(2, dim=1)                 # [B, out_channels, 1, 1]
            x = x * (gamma + 1) + beta                                # FiLM
        return x


class ConvUnet(Module):
    def __init__(
        self,
        dim,
        init_dim = None,
...
class TuckerUnet(Module):
    def __init__(
        ...
        """Tucker rank initialization"""
        self.low_rank_h = k1  
        self.low_rank_w = k2 
        self.height = H       
        self.width  = W

        # Column orthogonal matrices U (H, k1), V (W, k2)
        self.U = nn.Parameter(torch.randn(H, self.low_rank_h) * 0.02, requires_grad=True)
        self.V = nn.Parameter(torch.randn(W, self.low_rank_w) * 0.02, requires_grad=True)
    ...
    def forward(self, x, time, x_self_cond = None):
        ...
        # 1. The FiLM layer
        x = self.init_conv(x, time_emb=t)  # t: [B, time_emb_dim]  
    
        # 2. save r for skip connection
        r = x.clone()  # [B, init_dim, H, W]

        # 3. Tucker Encoder
        x = torch.einsum('bchw, hi -> bciw', x, self.U)  # [B, init_dim, k1, W]
        x = torch.einsum('bciw, wj -> bcij', x, self.V)  # [B, init_dim, k1, k2]
        ...
        # 5. Tucker Decoder
        x = torch.einsum('bcij, hi -> bchj', x, self.U)  # [B, C', H, k2]
        x = torch.einsum('bchj, wj -> bchw', x, self.V)  # [B, C', H, W]

        # 6. skip connection
        x = torch.cat((x, r), dim=1)  # [B, C' + init_dim, H, W]
...
class Trainer:
    ...
    def train(self):
        ...
        for epoch in range(self.train_epochs):
            ...
            with tqdm(total=len(self.dataloader), desc=f"Epoch {epoch+1}/{self.train_epochs}", disable=not self.accelerator.is_main_process) as pbar:
                ...
                if self.accelerator.is_main_process:
                    ...
                    self.epoch_times.append(time.perf_counter() - start_time)
        ...
        if self.accelerator.is_main_process:
            np.save(self.checkpoint_folder / "losses.npy", np.array(self.losses))
            np.save(self.checkpoint_folder / "epoch_times.npy", np.array(self.epoch_times))
