...
import argparse
import time
from TuckerUnet import TuckerUnet, GaussianDiffusion, Trainer
import config.TuckerUnet_config as config
...
def train_model_tucker(data_path, seed=None, num_samples=None, gpu_id=0, epochs=None,U_init=None,
        V_init=None,H=None,W=None,k1=None,k2=None,channels=None,train_UV=True):
    """
    ...
    if len(data_shape) == 2:
        ...
        data = torch.from_numpy(data_np).float()
        samples, channels1, height, width = data_shape
        print(f"Already in [N, C, H, W] format: {data.shape}")
    ...
    U_init_t = torch.as_tensor(U_init, dtype=model.U.dtype, device=model.U.device)
    V_init_t = torch.as_tensor(V_init, dtype=model.V.dtype, device=model.V.device)

    with torch.no_grad():
        model.U.copy_(U_init_t)
        model.V.copy_(V_init_t)

    model.U.requires_grad_(train_UV)
    model.V.requires_grad_(train_UV)
    ...
    total_sampling_time = end_sampling_time - start_sampling_time
    print(f"Sampling completed in {total_sampling_time:.2f} seconds")
